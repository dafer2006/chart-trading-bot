...
def panel(title):
    ...
    l.addWidget(lab(title,"Section")); return f,l


class Metric(QFrame):
    def __init__(self,title,value="—",sub=""):
        super().__init__(); self.setObjectName("Metric")
        l=QVBoxLayout(self); l.setContentsMargins(12,9,12,9); l.setSpacing(2)
        l.addWidget(lab(title,"MT")); self.value=lab(value,"MV"); self.sub=lab(sub)
        l.addWidget(self.value); l.addWidget(self.sub)


class MarketChart(QWidget):
    """Interactive native Qt chart: mouse wheel zoom, drag pan, double-click reset."""
    def __init__(self):
        super().__init__(); self.data={}; self.zoom=1.0; self.offset=0; self.drag_start=None
        self.setMinimumHeight(360); self.setMouseTracking(True)
    ...
    def _window(self):
        n=len(self.data.get("close") or []); visible=max(20,min(n,int(n/self.zoom))) if n else 0
        max_start=max(0,n-visible); start=max(0,min(max_start,max_start-self.offset)); return start,visible
    def wheelEvent(self,event):
        self.zoom=max(0.6,min(6.0,self.zoom*(1.18 if event.angleDelta().y()>0 else 1/1.18))); self.update()
    ...
    def mouseMoveEvent(self,event):
        if self.drag_start is not None and event.buttons() & Qt.LeftButton:
            dx=event.position().toPoint().x()-self.drag_start.x(); n=len(self.data.get("close") or [])
            start,visible=self._window(); px=max(3.0,(self.width()-40)/max(1,visible)); self.offset+=int(-dx/px)
            self.offset=max(0,min(max(0,n-visible),self.offset)); self.drag_start=event.position().toPoint(); self.update()
    ...
    def paintEvent(self,event):
        ...
        r=self.rect().adjusted(10,8,-10,-8); p.fillRect(r,QColor("#08111e"))
        closes=list(self.data.get("close") or []); opens=list(self.data.get("open") or []); highs=list(self.data.get("high") or []); lows=list(self.data.get("low") or [])
        vols=list(self.data.get("volume") or []); ema=list(self.data.get("ema50") or [])
        n=min(map(len,(closes,opens,highs,lows))) if closes else 0
        if n<2:
            p.setPen(QColor("#70839d")); p.drawText(r,Qt.AlignCenter,"Waiting for historical market data"); return
        start,visible=self._window(); end=min(n,start+visible)
        opens,highs,lows,closes=opens[start:end],highs[start:end],lows[start:end],closes[start:end]; vols=vols[start:end] if vols else []
        ema=ema[start:end] if len(ema)>=end else []; n=len(closes); lo=min(lows); hi=max(highs); span=hi-lo or 1.0
        chart_h=int(r.height()*0.74); chart=r.adjusted(0,0,0,-(r.height()-chart_h)); volume=r.adjusted(0,chart_h+6,0,0)
        def y(v): return int(chart.bottom()-((float(v)-lo)/span)*(chart.height()-22)-11)
        p.setPen(QPen(QColor("#1a2940"),1))
        for i in range(1,5):
            yy=int(chart.top()+chart.height()*i/5); p.drawLine(chart.left(),yy,chart.right(),yy)
        step=max(4.0,chart.width()/n); body=max(2,int(step*.58))
        for i in range(n):
            x=int(chart.left()+step*(i+.5)); yo,yc,yh,yl=y(opens[i]),y(closes[i]),y(highs[i]),y(lows[i]); c=QColor("#34d399" if closes[i]>=opens[i] else "#fb7185")
            p.setPen(QPen(c,1)); p.drawLine(x,yh,x,yl); p.fillRect(x-body//2,min(yo,yc),body,max(2,abs(yc-yo)),c)
        if len(ema)==n:
            pts=[]
        ...
        if vols:
            ...
            for i,v in enumerate(vols):
                x=int(volume.left()+step*(i+.5)); h=int((float(v)/vmax)*max(8,volume.height()-8)); p.fillRect(x-body//2,volume.bottom()-h,body,h,QColor("#28415f"))
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.thread=None; self.worker=None; self.broker_live=False; self.emergency_active=False; self.scan_cache={}; self.snapshot={}
        self._build_ui(); self.clock_timer=QTimer(self); self.clock_timer.timeout.connect(self.clock); self.clock_timer.start(1000); self.clock()

    def _build_ui(self):
        root=QWidget(); outer=QHBoxLayout(root); outer.setContentsMargins(0,0,0,0); outer.setSpacing(0); outer.addWidget(self._sidebar())
        content=QWidget(); main=QVBoxLayout(content); main.setContentsMargins(16,14,16,10); main.setSpacing(10); main.addWidget(self._topbar()); main.addWidget(self._metrics())
        ...
        main.addWidget(self.pages,1); main.addLayout(self._footer()); outer.addWidget(content,1); self.setCentralWidget(root)

    def _sidebar(self):
        s=QFrame(); s.setObjectName("Side"); s.setFixedWidth(190); l=QVBoxLayout(s); l.setContentsMargins(14,20,14,16); l.setSpacing(4)
        l.addWidget(lab("AI TRADER","Brand")); l.addWidget(lab("AUTOMATED EQUITY DESK","Sub")); l.addSpacing(18); self.nav=[]
        for i,name in enumerate(["Dashboard","Markets","Watchlist","Portfolio","Orders","AI Signals","Risk Control","Settings"]):
            b=QPushButton(name); b.setObjectName("Nav"); b.setCheckable(True); b.setChecked(i==0); b.clicked.connect(lambda _,btn=b,idx=i:self._nav(btn,idx)); self.nav.append(b); l.addWidget(b)
        l.addStretch(1); self.side_ibkr=lab("IBKR  OFFLINE","Bad"); self.side_tv=lab("TRADINGVIEW  OFFLINE"); l.addWidget(self.side_ibkr); l.addWidget(self.side_tv); l.addSpacing(8)
        e=QPushButton("■  EMERGENCY STOP"); e.setObjectName("Emergency"); e.clicked.connect(self.emergency_stop); l.addWidget(e); return s
    ...
    def _topbar(self):
        f=QFrame(); f.setObjectName("Top"); l=QHBoxLayout(f); box=QVBoxLayout(); self.page_title=lab("Dashboard","Title"); self.clock_label=lab(""); box.addWidget(self.page_title); box.addWidget(self.clock_label); l.addLayout(box); l.addStretch(1)
        self.search=QLineEdit(); self.search.setPlaceholderText("Search symbol…"); self.search.setFixedWidth(170); self.search.returnPressed.connect(lambda:self._use_symbol(self.search.text())); l.addWidget(self.search)
        self.symbol=QComboBox(); self.symbol.setEditable(True); self.symbol.addItem(settings.symbol); self.symbol.setFixedWidth(95); self.symbol.currentTextChanged.connect(self._symbol_changed); l.addWidget(self.symbol)
        self.timeframe=QComboBox(); self.timeframe.addItems(["1 min","5 mins","10 mins","15 mins","30 mins","1 hour","1 day"]); self.timeframe.setCurrentText(settings.timeframe); self.timeframe.currentTextChanged.connect(lambda v:setattr(settings,"timeframe",v)); self.timeframe.setFixedWidth(90); l.addWidget(self.timeframe)
        self.connect_btn=QPushButton("Connect IBKR"); self.connect_btn.setObjectName("Primary"); self.connect_btn.clicked.connect(self.connect_ibkr)
        self.start_btn=QPushButton("Start Scanner"); self.start_btn.setEnabled(False); self.start_btn.clicked.connect(self.start_scanner)
        self.pause_btn=QPushButton("Pause"); self.pause_btn.setEnabled(False); self.pause_btn.clicked.connect(self.pause_scanner)
        self.disconnect_btn=QPushButton("Disconnect"); self.disconnect_btn.setEnabled(False); self.disconnect_btn.clicked.connect(self.disconnect_ibkr)
        for x in (self.connect_btn,self.start_btn,self.pause_btn,self.disconnect_btn): l.addWidget(x)
        return f

    def _metrics(self):
        w=QWidget(); g=QGridLayout(w); g.setContentsMargins(0,0,0,0); g.setSpacing(9)
        self.m_ibkr=Metric("IBKR","DISCONNECTED","No broker snapshot"); self.m_pos=Metric("POSITIONS","—","Waiting for broker"); self.m_open=Metric("OPEN ORDERS","—","Waiting for broker"); self.m_exec=Metric("EXECUTED",f"0 / {settings.max_executed_orders}",f"Scope: {settings.execution_count_scope}"); self.m_gate=Metric("EXECUTION GATE","WAITING","Fail-closed"); self.m_sig=Metric("LATEST SIGNAL","—","AI / chart engine")
    ...
    def _dashboard_center(self):
        w=QWidget(); l=QVBoxLayout(w); l.setContentsMargins(0,0,0,0); l.setSpacing(10); p,pl=panel("MARKET VIEW")
        h=QHBoxLayout(); self.chart_symbol=lab("—","Title"); self.chart_price=lab("—","MV"); self.chart_change=lab("NO DATA"); h.addWidget(self.chart_symbol); h.addWidget(self.chart_price); h.addWidget(self.chart_change); h.addStretch(1); h.addWidget(lab("Wheel: zoom  •  Drag: pan  •  Double click: reset")); pl.addLayout(h)
        controls=QHBoxLayout(); reset=QPushButton("Reset Chart"); reset.clicked.connect(lambda:self.chart.reset()); controls.addWidget(reset); self.chart_info=lab("Candles 120  •  EMA50  •  Volume"); controls.addWidget(self.chart_info); controls.addStretch(1); pl.addLayout(controls)
        self.chart=MarketChart(); pl.addWidget(self.chart,1); self.ctx={}
        grid=QGridLayout()
        for i,(title,key) in enumerate([("EMA50","ema50"),("Williams %R","williams_r"),("MFI 14","mfi14"),("Volume Ratio","volume_ratio"),("Cloud Top","cloud_top"),("Cloud Bottom","cloud_bottom")]):
            box=QFrame(); box.setObjectName("Metric"); bl=QVBoxLayout(box); bl.setContentsMargins(9,7,9,7); bl.addWidget(lab(title,"MT")); v=lab("—","MV"); self.ctx[key]=v; bl.addWidget(v); grid.addWidget(box,i//3,i%3)
        pl.addLayout(grid); self.cloud_label=lab("Cloud: —"); pl.addWidget(self.cloud_label); l.addWidget(p,3)
        p2,p2l=panel("SCANNER & RISK"); form=QFormLayout(); self.interval=QComboBox(); self.interval.addItems(["15","30","60","120","300"]); self.interval.setCurrentText(str(settings.scan_interval_seconds)); self.quantity=QSpinBox(); self.quantity.setRange(1,1000000); self.quantity.setValue(settings.fixed_quantity); self.tp=QDoubleSpinBox(); self.tp.setRange(.1,500); self.tp.setDecimals(2); self.tp.setValue(settings.take_profit_percent); form.addRow("Scan interval (sec)",self.interval); form.addRow("Order quantity",self.quantity); form.addRow("Take profit (%)",self.tp); self.limit_info=lab(self._limits_text()); form.addRow("Limits",self.limit_info); p2l.addLayout(form); l.addWidget(p2)
        p3,p3l=panel("LIVE SCANNER RESULTS"); self.scan_table=self._table(["Symbol","Signal","Score","Entry","Stop","Target","TP %","Status"]); self.scan_table.cellClicked.connect(lambda r,c:self._use_symbol(self.scan_table.item(r,0).text()) if self.scan_table.item(r,0) else None); p3l.addWidget(self.scan_table); l.addWidget(p3,2); return w

    def _dashboard_right(self):
        w=QWidget(); l=QVBoxLayout(w); l.setContentsMargins(0,0,0,0); l.setSpacing(10); p,pl=panel("EXECUTION GATE"); self.gate_status=lab("WAITING","Warn"); self.gate_reason=lab("Connect IBKR before any execution decision."); self.gate_reason.setWordWrap(True); self.gate_progress=QProgressBar(); self.gate_progress.setRange(0,settings.max_active_positions); self.gate_progress.setValue(0); pl.addWidget(self.gate_status); pl.addWidget(self.gate_progress); pl.addWidget(self.gate_reason); self.gate_stats=lab("Positions —  •  Open Orders —  •  Executed 0/7"); pl.addWidget(self.gate_stats); l.addWidget(p)
        p,pl=panel("LATEST AI ANALYSIS"); self.ai_fields={}
        for title,key in [("Signal","signal"),("Score","score"),("Entry","entry"),("Stop","stop"),("Target","target"),("R:R","rr"),("Reasons","reasons"),("Execution","execution")]:
            row=QHBoxLayout(); row.addWidget(lab(title,"MT")); v=lab("—"); v.setWordWrap(True); row.addWidget(v,1); self.ai_fields[key]=v; pl.addLayout(row)
    ...
    def _portfolio_page(self):
        page=QWidget(); l=QVBoxLayout(page); g=QGridLayout(); self.p_account=Metric("NET LIQUIDATION","—","IBKR"); self.p_positions=Metric("POSITIONS","—","Active"); self.p_open=Metric("OPEN ORDERS","—","Broker"); self.p_executed=Metric("EXECUTED","0/7",settings.execution_count_scope)
        for i,x in enumerate([self.p_account,self.p_positions,self.p_open,self.p_executed]): g.addWidget(x,0,i)
        l.addLayout(g); p,pl=panel("POSITIONS"); self.positions_table=self._table(["Symbol","Quantity","Side","Source"]); pl.addWidget(self.positions_table); l.addWidget(p,1); return page
    ...
    def connect_ibkr(self):
        ...
        self.connect_btn.setEnabled(False); self.disconnect_btn.setEnabled(True); self.footer_status.setText("Connecting to IBKR…")
    ...
    def on_snapshot(self,data):
        self.snapshot=data or {}; pos=self.snapshot.get("positions") or []; opens=self.snapshot.get("open_orders") or []; exe=int(self.snapshot.get("executed") or 0); mx=int(self.snapshot.get("maximum") or settings.max_executed_orders); account=self.snapshot.get("account_value")
        self.m_pos.value.setText(str(len(pos))); self.m_pos.sub.setText("LIVE • fresh snapshot"); self.m_open.value.setText(str(len(opens))); self.m_open.sub.setText("LIVE • fresh snapshot"); self.m_exec.value.setText(f"{exe} / {mx}"); self.m_exec.sub.setText(f"Scope: {settings.execution_count_scope}")
        self.gate_progress.setRange(0,max(1,settings.max_active_positions)); self.gate_progress.setValue(min(len(pos),settings.max_active_positions)); self.gate_stats.setText(f"Positions {len(pos)}/{settings.max_active_positions}  •  Open Orders {len(opens)}/{settings.max_open_orders}  •  Executed {exe}/{mx}")
        ...
        self.risk_pos.value.setText(f"{len(pos)} / {settings.max_active_positions}"); self.risk_open.value.setText(f"{len(opens)} / {settings.max_open_orders}"); self.risk_exec.value.setText(f"{exe} / {mx}"); self.p_positions.value.setText(f"{len(pos)} / {settings.max_active_positions}"); self.p_open.value.setText(f"{len(opens)} / {settings.max_open_orders}"); self.p_executed.value.setText(f"{exe} / {mx}")
        self.p_account.value.setText("—" if account is None else f"${float(account):,.2f}"); self.p_account.sub.setText("IBKR Net Liquidation" if account is not None else "Waiting for broker")
    ...
    def on_scan(self,data):
        ...
        for k,v in self.ctx.items():
            value=context.get(k); v.setText("—" if value is None else f"{float(value):.4f}")
        self.cloud_label.setText("Cloud: BELOW — bearish structure" if context.get("below_cloud") else "Cloud: ABOVE — bullish structure" if context.get("above_cloud") else "Cloud: NEUTRAL")
        self.ai_fields["signal"].setText(str(signal.action)); self.ai_fields["score"].setText(str(signal.score)); self.ai_fields["entry"].setText(f"{signal.entry:.4f}"); self.ai_fields["stop"].setText(f"{signal.stop:.4f}" if signal.stop is not None else "—"); self.ai_fields["target"].setText(f"{signal.target:.4f}" if signal.target is not None else "—")
        if signal.stop and signal.target:
            risk=abs(signal.entry-signal.stop); reward=abs(signal.target-signal.entry); self.ai_fields["rr"].setText(f"1 : {reward/risk:.2f}" if risk else "—")
    ...
    def _upsert_scan(self,table,data):
        symbol=str(data["symbol"]); signal=data["signal"]; status="BUY CANDIDATE" if signal.action=="BUY" else "SELL HELD" if signal.action=="SELL" else "HOLD"
        row=next((r for r in range(table.rowCount()) if table.item(r,0) and table.item(r,0).text()==symbol),-1)
    ...
    def on_order(self,r):
        vals=[r.time,r.symbol,r.action,str(r.quantity),f"{r.entry_limit:.4f}",f"{r.stop:.4f}" if r.stop else "—",f"{r.target:.4f}" if r.target else "—",r.status]
        self.recent_orders.insertRow(0)
